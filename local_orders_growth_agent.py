...
DEFAULT_TARGET_URL = (
    "https://bigdata.intra.didiglobal.com/analysis_platform_static/board.html"
    "#/?type=reportView&reportId=231141&source=market&subReportId=377455"
)
DEFAULT_PROFILE_DIR = os.path.join(r"C:\ImperioOrdersGrowth", "chrome_user_data")
...
def _html_page(params):
    ...
    period = html.escape((params.get("period", [""])[0]).strip())
...
<head>
  ...
  <title>Coletor Orders Growth - Imperio</title>
  <style>
    :root {{
      --bg: #070707;
...
<body>
  <div class="wrap">
    <div class="head">
      <h2>Coletor Orders Growth (sem PowerShell)</h2>
...
class OrdersGrowthAgentHandler(BaseHTTPRequestHandler):
...
def main():
    parser = argparse.ArgumentParser(description="Agente local para coleta do Orders Growth no Windows.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8766)
    args = parser.parse_args()

    server = ThreadingHTTPServer((args.host, args.port), OrdersGrowthAgentHandler)
    print(f"[orders-growth-agent] Rodando em http://{args.host}:{args.port}/")
    print(f"[orders-growth-agent] Versao: {VERSION}")
    print("[orders-growth-agent] Abra essa URL no navegador e use os botoes.")
    try:
